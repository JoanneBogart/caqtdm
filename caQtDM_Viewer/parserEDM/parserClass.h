...
enum classType {activeLine, activeRectangle, activeCircle, activeArc ,
                xyGraph, activePip, activeXText, activeXTextDsp, TextUpdate, 
                TextEntry, Byte, // Zai added three items
                activeMeter, activeBar, activeMessageBox,
                activeMotifSlider, activeButton, activeMenuButton,
                activeRadioButton, activeChoiceButton, // Zai added one item
                activeMessageButton, activeExitButton, relatedDisplay,
                shellCmdButton, activeGroup, activePngImage,
               unknown};
...
public:
  ...
  void incLine ( void );
  void addVisibilityCalc(myParserEDM *myParser, char *visPvExpStr,
...
private:
  ...
  char fontTag[80] ;
  int left;
  int fgColor, bgColor, fgColorMode, bgColorMode, visInverted, autoSize,
    border, lineThk;
  int zero;
  double dzero;
  int one ;
  ...
  int formatType, colormode, editable, autoHeight, isWidget, limitsFromDb,
    colorMode, index;
  int smartRefresh, useKp, changeValOnLoseFocus, fastUpdate, isDate, isFile,
    autoSelect;
  int updatePvOnDrop, useHexPrefix,fileComponent, dateAsFileName, showUnits,
    useAlarmBorder;
  int newPositioning, inputFocusUpdatesAllowed, clipToDspLimits, isPassword,
    characterMode;
  ...
  int lineColor, fillColor, lineColorMode, fill, fillColorMode, lineWidth,
    lineStyle, invisible;
  ...
  expStringClass readPvExpStr,  readMinExpStr, readMaxExpStr, nullPvExpStr,
    label, precisionExpStr ;
  expStringClass labelTicksExpStr, majorTicksExpStr, minorTicksExpStr,
    barOriginValExpStr;
  char scaleFormat[15+1];

  // reading activeMeterClass
  int meterColorMode, meterColor, scaleColor, scaleColorMode, labelColor,
    tsColor, bsColor;
  int trackDelta, labelIntervals, majorIntervals, minorIntervals, needleType,
    shadowMode;
  int scaleLimitsFromDb;
  expStringClass controlPvExpStr, scaleMinExpStr,scaleMaxExpStr,
    scalePrecExpStr, labIntExpStr;
  char literalLabel[39+1], scaleFontTag[63+1], labelFontTag[63+1];
  double meterAngle;
  double a180;

  // reading  activeMotifSliderClass
  int shadeColor, topColor, botColor, increment, controlLabelType, showLimits,
    showLabel, showValue;
...
#define XYGC_K_MAX_TRACES 20
  ...
  int plotAreaBorder, autoScaleBothDirections, autoScaleTimerMs,
    autoScaleThreshPct;
  ...
  int plotColor[XYGC_K_MAX_TRACES], xylineThk[XYGC_K_MAX_TRACES],
    xylineStyle[XYGC_K_MAX_TRACES];
  int plotSymbolType[XYGC_K_MAX_TRACES], plotUpdateMode[XYGC_K_MAX_TRACES],
    opMode[XYGC_K_MAX_TRACES];
  int xSigned[XYGC_K_MAX_TRACES], ySigned[XYGC_K_MAX_TRACES];
  efDouble xMin, xMax;
  efInt xNumLabelIntervals, xNumMajorPerLabel,  xNumMinorPerMajor,
    xAnnotationPrecision ;
  int xLabelGrid, xMajorGrid, xMinorGrid, xAnnotationFormat, xGridMode,
    xAxisSmoothing;
  ...
  int y1AnnotationFormat[NUM_Y_AXES], y1GridMode[NUM_Y_AXES],
    y1AxisSmoothing[NUM_Y_AXES];
  ...
  expStringClass sourcePressPvExpString, sourceReleasePvExpString,
    destPvExpStringM, visPvExpString;
  ...
  // verify the actual x and y for widgets inside groups. Necessary because
  // while EDM widgets work with absolute values
